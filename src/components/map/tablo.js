...
// Событие перехода на другой регион с основной карты
export function gotoRegionMap(uid) {
  let node = `nod${uid}`;
  ...
  if (node != null) {
    ...
    if (typeof url !== undefined) {
      ...
      const layers = parentProps.showLayer;
      if (layers) {
        const dncOnLayer = layers.map((item) => {
          return {
            ...item,
            data: item.data.map((t) => {
              if (t.layer.includes('dnc')) {
                return {
                  ...t,
                  show: true,
                };
        ...
        parentProps.postShowLayer(dncOnLayer);
      }
...
// Клик на иконки шторма на основной карте. Переход на регион
...
// Клик по иконки шторма в регионе
